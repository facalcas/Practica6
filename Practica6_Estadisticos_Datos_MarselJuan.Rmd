...
Datos%>%
  arrange(ALG.num)
#Además se pueden visualizar abriendo el conjunto Datos
```
...
Datos%>%
  pivot_longer(names_to = "asignatura",values_to = "nota",cols = ends_with(".num"))%>%
  group_by(asignatura)%>%
  summarise(mean=mean(nota,na.rm=T),
            sd=sd(nota,na.rm=T),
            median=median(nota,na.rm=T),
            iqr=IQR(nota,na.rm=T))
```
...
Datos%>%
  ...
  ggplot(data=.)+geom_histogram(aes(x=nota,y=stat(density)))+
  geom_density(aes(x=nota))+
  geom_vline(aes(xintercept = mean(nota,na.rm=T)),color="red",size=1)
```
...
AL<-table(Datos$ALG.cat)
AL
ALGyANM<-table(Datos$ALG.cat,Datos$ANM.cat)
ALGyANM

mosaicplot(x = ALGyANM,xlab = "Nota ALG",ylab = "Nota ANM")
```
...
chisq.test(x=Datos$ALG.cat,y=Datos$ANM.cat,correct = F)

```


```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
chisq.test(x=Datos$ALG.cat,y=Datos$Sex,correct = F)
